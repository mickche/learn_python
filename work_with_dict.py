...
def get_hobbies() -> list[str]:
    hobbies = []
    for h in range(3):
        hobbies.append(input(f"What is your favorite hobby #{h+1}: "))
    return hobbies


def add_data_to_users(users: dict) -> dict:
    nickname = input("Enter your nickname plz: ")
    users[nickname] = {"name": get_name(), "age": get_age(), "hobbies": get_hobbies()}
    return users
...
def print_users_table(users: dict):
    ...
    for nickname, info in users.items():
        ...
        table.add_row(nickname, info["name"], str(info["age"]), hobbies_str)
    return table
